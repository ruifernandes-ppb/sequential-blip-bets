import { Trophy, Clock, ArrowRight, Users, Target, Receipt } from 'lucide-react';
...
export function EventSelection({ onMatchSelect, onViewFriendsStats, onPenaltyShootout, onViewMyBets }: EventSelectionProps) {
  const { placedBets } = useBets();
  const activeBetsCount = placedBets.filter(bet => bet.status === 'pending' || bet.status === 'live').length;
  ...
  return (
    <div className="min-h-screen flex flex-col p-4 max-w-md mx-auto">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  {activeBetsCount > 0 && (
                    <span className="text-cyan-400 text-xs">{activeBetsCount} active bet{activeBetsCount !== 1 ? 's' : ''}</span>
                  )}
                </div>
              </div>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-gray-400 text-sm">{UPCOMING_MATCHES.length} matches</span>
      </div>
