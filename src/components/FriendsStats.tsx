import { ArrowLeft, Trophy, TrendingUp, Target, Flame, DollarSign, Award, Medal } from 'lucide-react';
...
export function FriendsStats({ onBack }: FriendsStatsProps) {
  ...
  const topByWinRate = [...FRIENDS_DATA].sort((a, b) => b.winRate - a.winRate).slice(0, 3);
  const topByStreak = [...FRIENDS_DATA].sort((a, b) => b.currentStreak - a.currentStreak).slice(0, 3);
          ...
          <div className="space-y-1">
            {topByWinRate.map((friend, idx) => (
              <div key={friend.id} className="flex items-center gap-2">
                ...
                <span className="text-white text-xs flex-1">{friend.name.split(' ')[0]}</span>
                <span className="text-green-400 text-xs">{friend.winRate}%</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-1">
            {topByStreak.map((friend, idx) => (
              <div key={friend.id} className="flex items-center gap-2">
                <span className="text-lg">{friend.avatar}</span>
                <span className="text-white text-xs flex-1">{friend.name.split(' ')[0]}</span>
                <span className="text-orange-400 text-xs">{friend.currentStreak}🔥</span>
              </div>
            ))}
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-3">
                  {/* Rank */}
                  <div className={`flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br ${getRankColor(friend.rank)} flex-shrink-0`}>
                    {friend.rank <= 3 ? (
                      <span className="text-white">{getRankIcon(friend.rank)}</span>
                    ) : (
                  ...
                  <div className="text-right">
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <span className="text-green-400">{friend.totalWinnings.toFixed(2)}</span>
                    </div>
                    <p className="text-gray-400 text-xs">{friend.winRate}% win rate</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 pt-3 border-t border-gray-700">
                  <div className="bg-[#0f1f3d] rounded-lg p-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-white text-sm">{friend.currentStreak}</span>
                    </div>
                  </div>
                  <div className="bg-[#0f1f3d] rounded-lg p-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-white text-sm">{friend.longestStreak}</span>
                    </div>
                  </div>
                  <div className="bg-[#0f1f3d] rounded-lg p-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-white text-sm">{friend.winRate}%</span>
                    </div>
                  </div>
                </div>
              </div>
