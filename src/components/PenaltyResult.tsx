    ...
    <div className="min-h-screen flex flex-col p-4 max-w-md mx-auto">
      ...
      <div className="space-y-3 mb-6">
        ...
        <div className={`rounded-2xl p-6 border-2 ${
          ...
          <div className="flex items-center gap-2">
            <span className={`text-sm ${
              ...
              {isProfitable ? '+' : ''}{netProfit.toFixed(2)} ({isProfitable ? '+' : ''}{((netProfit / initialStake) * 100).toFixed(0)}%)
            </span>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-[#1a2f4d] to-[#0f1f3d] rounded-2xl p-6 border border-cyan-500/30">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-cyan-400 text-2xl">{winRate.toFixed(0)}%</span>
          </div>
      ...
      <div className="bg-[#1a2f4d]/50 rounded-xl p-4 mb-6 border border-cyan-500/20">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-red-400 text-sm">{totalPenalties - correctPredictions}</span>
          </div>
      ...
      <div className="bg-gradient-to-br from-purple-900/40 to-blue-900/40 rounded-xl p-4 mb-6 border border-purple-500/30">
        <p className="text-white text-center">
          {winRate >= 80 ? '🏆 Outstanding! You\'re a penalty prediction master!' :
           winRate >= 70 ? '⭐ Excellent work! Great prediction skills!' :
           winRate >= 60 ? '👍 Good job! You\'re getting the hang of it!' :
           winRate >= 50 ? '💪 Not bad! Keep practicing!' :
      ...
      <div className="space-y-3 mt-auto">
        <Button
          onClick={onPlayAgain}
          className="w-full bg-gradient-to-r from-cyan-400 to-green-400 hover:from-cyan-500 hover:to-green-500 text-[#0a1628] py-6 rounded-2xl"
        >
          <div className="flex items-center justify-center gap-2">
            <Trophy className="w-5 h-5" />
            <span>New Penalty Shootout</span>
          </div>
        </Button>
